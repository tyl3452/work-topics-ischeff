...
def build_markov(corpus):
    corpus.split()#split the string input into a list of words
    markov = {}#create an empty dictionary to hold the model - keys and values are words
    for i in range(len(corpus)):#should these be reversed?
        current_word = corpus[i]
        # previous_words = ' '.join(corpus[i-1])
...
if __name__ == "__main__":
    ...
    print(body)
    model = build_markov(body)#this seems to be making a model based on characters, not words
    print(model)
